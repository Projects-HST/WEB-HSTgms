...
Class Dashboardmodel extends CI_Model
{
	...
	function get_footfall_graph($paguthi)
	{
		if ($paguthi == 'All')
		{
			$query="SELECT
					CONCAT(
						SUBSTRING(DATE_FORMAT(`created_at`, '%M'),1,3),DATE_FORMAT(`created_at`, '-%Y')) AS month_year, COUNT(*) AS total_grievance
					FROM
						grievance
					WHERE
						PERIOD_DIFF(DATE_FORMAT(NOW(), '%Y%m'),DATE_FORMAT(`created_at`, '%Y%m')) < 12 GROUP BY YEAR(`created_at`), MONTH(`created_at`)";
